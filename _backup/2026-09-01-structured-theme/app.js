(function () {
  ...
  async function generateThemes() {
    ...
    const system = [
      '你是一位资深音乐创作灵感助手。用户会给出一个音乐风格描述，请按以下要求构思歌曲主题：',
      '1. 首先用一句话精准定义该风格的核心气质（例如："镀了金的怀旧与致幻的孤独感"）。这句话用于 anchoring 你的整体判断，必须一并返回。',
      '2. 然后围绕不同的场景与情绪维度，为该风格构思 5 个歌曲主题。要求立意新颖、避免陈词滥调，并具有一定的哲学或情感深度。',
      '3. 每个主题严格按以下结构输出：',
      '   - name：主题名称（例如："盛夏的黄昏与逝去的青春"），中文，不超过 15 字；',
      '   - scene：适用场景，描述一个具体、富有画面感的场景；',
      '   - imagery：歌词意象，3-5 个核心关键词或意象的数组（例如：["百叶窗光影","泳池反光"]）；',
      '   - audio：听觉描述，说明期望的混响、质感或人声效果；',
      '   - mood：情绪基调，用一个词或短语概括。',
      '4. 所有主题、意象与听觉描述必须贴合用户给出的音乐风格关键词（取自风格本身），不得偏离或虚构无关元素。',
      '只返回 JSON，不要有其他任何文字，格式：',
      '{"styleTemperament":"一句话核心气质","themes":[{"name":"…","scene":"…","imagery":["…","…"],"audio":"…","mood":"…"}]}（themes 数组恰好 5 个元素）'
    ].join('\n');
    const user = '音乐风格（英文）：' + s.en + '\n音乐风格（中文）：' + s.cn + '\n风格关键词（取自上述风格，不得虚构）：' + s.en + '\n请按结构为这个风格生成 5 个主题。';
    ...
    // 统一规范化为结构化主题对象（兼容模型返回纯字符串的降级场景）
    const normalized = themes.slice(0, 5).map(t => {
      if (t && typeof t === 'object') {
        return {
          name: String(t.name || '').trim(),
          scene: String(t.scene || '').trim(),
          imagery: Array.isArray(t.imagery) ? t.imagery.map(String).filter(Boolean) : [],
          audio: String(t.audio || '').trim(),
          mood: String(t.mood || '').trim()
        };
      }
      return { name: String(t || '').trim(), scene: '', imagery: [], audio: '', mood: '' };
    }).filter(t => t.name);
    const temperament = obj && obj.styleTemperament ? String(obj.styleTemperament).trim() : '';
    return { temperament, themes: normalized };
  }

  function themeName(t) {
    return (t && typeof t === 'object') ? String(t.name || '') : String(t || '');
  }
  ...
  async function generateStories() {
    ...
    const themeBlock = (t && typeof t === 'object')
      ? '选定主题：' + themeName(t) + '\n' +
        '适用场景：' + (t.scene || '（无）') + '\n' +
        '歌词意象：' + ((t.imagery && t.imagery.length) ? t.imagery.join('、') : '（无）') + '\n' +
        '听觉描述：' + (t.audio || '（无）') + '\n' +
        '情绪基调：' + (t.mood || '（无）')
      : '选定主题：' + themeName(t);
    const user = '音乐风格（英文）：' + s.en + '\n音乐风格（中文）：' + s.cn + '\n' + themeBlock + '\n请创作3段不同的故事，每段约200字，贴合上述主题的场景、意象与情绪基调。';
    const raw = await callAI(system, user);
    const obj = extractJSON(raw);
  ...
  function themeCardHTML(t, i) {
    const lines = [];
    if (t.scene) lines.push('<div class="theme-line theme-scene">📍 ' + escapeHtml(t.scene) + '</div>');
    if (t.imagery && t.imagery.length) lines.push('<div class="theme-line theme-imagery">✨ ' + escapeHtml(t.imagery.join(' · ')) + '</div>');
    if (t.audio) lines.push('<div class="theme-line theme-audio">🎧 ' + escapeHtml(t.audio) + '</div>');
    if (t.mood) lines.push('<div class="theme-line theme-mood">💭 ' + escapeHtml(t.mood) + '</div>');
    return '<button class="card theme-card" data-i="' + i + '">' +
        '<div class="card-cn theme-name">' + escapeHtml(t.name) + '</div>' +
        lines.join('') +
      '</button>';
  }
  ...
  function renderStep2() {
    ...
    generateThemes().then(result => {
      if (state.step !== 2) return; // 期间已被重置
      state.styleTemperament = result.temperament;
      state.themeOptions = result.themes;
      const cards = state.themeOptions.map(themeCardHTML).join('');
      const temperamentHTML = state.styleTemperament
        ? '<p class="hint">风格气质：<b>' + escapeHtml(state.styleTemperament) + '</b></p>'
        : '';
      $content.innerHTML =
        '<section class="panel">' +
      ...
      document.getElementById('refresh-theme').addEventListener('click', () => {
        renderStep2(); // 保持当前风格，重新生成（内部含 loading 与错误处理）
      });
  ...
  function renderStep3() {
    renderStepper();
    showLoading('正在根据「' + state.selectedStyle.cn + ' / ' + themeName(state.selectedTheme) + '」创作 3 个故事…');
  ...
  function renderStep3Content() {
    ...
    const themeDetailHTML = (theme && typeof theme === 'object')
      ? [
          theme.scene ? '<div class="final-theme-detail">📍 ' + escapeHtml(theme.scene) + '</div>' : '',
          (theme.imagery && theme.imagery.length) ? '<div class="final-theme-detail">✨ ' + escapeHtml(theme.imagery.join(' · ')) + '</div>' : '',
          theme.audio ? '<div class="final-theme-detail">🎧 ' + escapeHtml(theme.audio) + '</div>' : '',
          theme.mood ? '<div class="final-theme-detail">💭 ' + escapeHtml(theme.mood) + '</div>' : ''
        ].filter(Boolean).join('')
      : '';
    ...
    $content.innerHTML =
      '<section class="panel final-panel">' +
        ...
        '<div class="final-theme">' +
          ...
          '<div class="final-theme-cn">' + escapeHtml(themeName(theme)) + '</div>' +
          themeDetailHTML +
        '</div>' +
